...
QBCore.Functions.CreateCallback('qb-banking:server:saveLanguage', function(source, cb, data)
    ...
    if lang ~= 'nl' and lang ~= 'en' then
        lang = 'nl'
    end

    local metadata = Player.PlayerData.metadata or {}
    metadata.bankLanguage = lang
    Player.Functions.SetMetaData('bankLanguage', lang)

    cb({ success = true, language = lang })
end)
...
QBCore.Functions.CreateCallback('qb-banking:server:saveSettings', function(source, cb, data)
    ...
    cb({ success = true, language = lang, theme = theme })
end)
...
QBCore.Functions.CreateCallback('qb-banking:server:payInvoice', function(source, cb, data)
    ...
    if not invoiceId then return cb({ success = false, message = Lang:t('error.error') }) end
    ...
    if not invoice then return cb({ success = false, message = Lang:t('error.error') }) end

    local amount = tonumber(invoice.amount) or 0
    if Player.PlayerData.money.bank < amount then return cb({ success = false, message = Lang:t('error.money') }) end

    Player.Functions.RemoveMoney('bank', amount, 'paid-invoice')
    if invoice.society and Accounts[invoice.society] then
        Accounts[invoice.society].account_balance = (Accounts[invoice.society].account_balance or 0) + amount
        MySQL.update.await('UPDATE bank_accounts SET account_balance = account_balance + ? WHERE account_name = ?', { amount, invoice.society })
    end
    CreateBankStatement(src, 'checking', amount, invoice.sender or invoice.society or 'Invoice', 'withdraw', 'player')
